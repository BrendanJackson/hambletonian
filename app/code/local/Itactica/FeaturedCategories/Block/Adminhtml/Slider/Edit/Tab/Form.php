...
class Itactica_FeaturedCategories_Block_Adminhtml_Slider_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm(){
        ...
        $fieldset->addField('minimum_width', 'text', array(
            ...
            'name'  => 'minimum_width',
            'note'	=> $this->__('Minimum width in pixels for each category box. Only applies to medium and large screens.'),
            'class'     => 'validate-number',
            'required' => true,

        ));

        $fieldset->addField('number_items_mobile', 'select', array(
            'label' => Mage::helper('itactica_featuredcategories')->__('Number of Items on Mobile'),
            'name'  => 'number_items_mobile',
            'note'	=> $this->__('This is the number of categories per row on mobile. Only screens up to 640px wide.'),
            'class' => 'required-entry',
            'values'=> array(
                array(
                    'value' => 1,
