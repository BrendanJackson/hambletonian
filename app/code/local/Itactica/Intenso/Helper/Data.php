	...
	public function getAlternativeImgUrl($product, $width, $height, $isLazyLoading = false) {
		...
		if ($altImg = $product->load('media_gallery')->getMediaGalleryImages()->getItemByColumnValue($column, $value)) {
			if ($imgFile = $altImg->getFile()) {
				if ($isLazyLoading) {
					return '<img class="alt-image" src="'.Mage::getDesign()->getSkinUrl('images/clear.png').'" data-echo="'.Mage::helper('catalog/image')->init($product, 'small_image', $imgFile)->resize($width, $height).'" width="'.$width.'" height="'.$height.'">';
				} else {
					return '<img class="alt-image" src="'.Mage::helper('catalog/image')->init($product, 'small_image', $imgFile)->resize($width, $height).'" width="'.$width.'" height="'.$height.'">';
				}
			}
		}
	}
